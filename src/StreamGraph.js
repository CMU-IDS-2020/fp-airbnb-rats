...
class StreamGraph extends Component {
  render() {
    ...
    const xScale = scaleLinear().domain([0, 30]).range([0, this.props.size[0]]);

    const yScale = scaleLinear().domain([0, 60]).range([this.props.size[1], 0]);
    ...
    return (
      <div>
        ...
        <StatLine allData={this.props.appdata} filteredData={this.props.data} />
      </div>
    );
  }
}
